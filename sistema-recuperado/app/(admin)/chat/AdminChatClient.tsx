...
export function AdminChatClient({ initialConversations }: AdminChatClientProps) {
    ...
    const {
        ...
        unsubscribe
    } = useChat({ isAdmin: true, initialConversations });

    // Initial setup
    useEffect(() => {
        // If we have initial conversations, we might skip the first fetch or merge
        // But useChat handles it if we pass initialConversations
        subscribeToConversationsList();

        return () => {
            unsubscribe();
        };
    }, [subscribeToConversationsList, unsubscribe]);
